...
- Fixed duplicate screensavers when the in-app screensaver is enabled by disabling the Home idle media banner and the Android TV dream service while the menu-configured screensaver is active.
...
- Fixed Home so backing out of an item no longer shifts focus left in the row you opened it from, by skipping the initial row-restore pass after detail return, suppressing horizontal row scroll until the next key press, and avoiding re-scroll when the focused item is already visible.
- Fixed Movies and TV Shows recommended tabs so backing out of an item no longer jumps focus up a row, using the same detail-return focus restore and vertical scroll suppression as Home.
