...
val extensionsRepoActive =
    (
        providers.gradleProperty("WholphinExtensionsUsername").orNull.isNullOrBlank().not() &&
            providers.gradleProperty("WholphinExtensionsPassword").orNull.isNullOrBlank().not()
    ) || (
        System.getenv("WHOLPHIN_EXTENSIONS_USERNAME").isNullOrBlank().not() &&
            System.getenv("WHOLPHIN_EXTENSIONS_PASSWORD").isNullOrBlank().not()
    ) || (
        System.getenv("GITHUB_ACTOR").isNullOrBlank().not() &&
            System.getenv("GITHUB_TOKEN").isNullOrBlank().not()
    )
...
android {
    ...
    signingConfigs {
        val debugSigningStoreFile = localProperties["debug.signing.storeFile"]?.toString()
        if (debugSigningStoreFile != null) {
            getByName("debug") {
        ...
        applicationVariants.all {
            ...
            variant.outputs
                ...
                .forEach { output ->
                    ...
                    val outputFileName =
                        "Ndorfin-${variant.baseName}-${variant.versionName}-${variant.versionCode}$abi.apk"
...
dependencies {
    ...
    if (ffmpegModuleExists) {
        ...
        logger.info("Using public Jellyfin Media3 ffmpeg decoder")
    ...
    if (av1ModuleExists) {
        ...
        logger.warn("Media3 av1 decoder was NOT found. Add app/libs/lib-decoder-av1-release.aar or configure Wholphin extension package credentials.")
    }
    if (mpvModuleExists) {
        ...
        logger.warn("libMPV was NOT found. Add app/libs/wholphin-mpv-release.aar or configure Wholphin extension package credentials.")
    }
