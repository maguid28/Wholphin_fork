...
//                                                ?.removePrefix(" - ")
        ...
        suspend fun getTrailers(item: BaseItem): List<Trailer> {
            val fullItem = BaseItem.from(api.userLibraryApi.getItem(item.id).content, api)
            val remoteTrailers = getRemoteTrailers(fullItem)
            val localTrailers = getLocalTrailers(fullItem)
